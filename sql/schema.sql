...
select
  ...
  case
    ...
    when tk.title = 'แผนการสอนหน่วยที่ 3' and t.name = 'ครูวิชัย ศรีสุข'   then 'open'
    when tk.title = 'รายงานผลการปฏิบัติงาน (SAR)' and t.name = 'ครูสมชาย ใจดี'   then 'sent'
    when tk.title = 'รายงานผลการปฏิบัติงาน (SAR)' and t.name = 'ครูสุนีย์ พรหมมา' then 'open'
    when tk.title = 'รายงานผลการปฏิบัติงาน (SAR)' and t.name = 'ครูวิชัย ศรีสุข'   then 'review'
    when tk.title = 'คะแนนเก็บกลางภาค' and t.name = 'ครูสมชาย ใจดี'   then 'sent'
    when tk.title = 'คะแนนเก็บกลางภาค' and t.name = 'ครูสุนีย์ พรหมมา' then 'late'
    when tk.title = 'คะแนนเก็บกลางภาค' and t.name = 'ครูวิชัย ศรีสุข'   then 'open'
    else 'open'
  end,
  case
    when tk.title = 'แผนการสอนหน่วยที่ 3' and t.name = 'ครูสมชาย ใจดี'   then now() - interval '2 days'
    when tk.title = 'รายงานผลการปฏิบัติงาน (SAR)' and t.name = 'ครูสมชาย ใจดี'   then now() - interval '1 day'
    when tk.title = 'คะแนนเก็บกลางภาค' and t.name = 'ครูสมชาย ใจดี'   then now() - interval '5 days'
    else null
  end
from teachers t
cross join tasks tk
on conflict (teacher_id, task_id) do nothing;
